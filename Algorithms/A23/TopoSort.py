...
class Graph (object):
    ...
    def add_directed_edge (self, start, finish, weight = 1):
        ...
        self.vertices[finish].in_degrees += 1
    ...
    def add_undirected_edge (self, start, finish, weight = 1):
        ...
        self.vertices[start].in_degrees += 1
    ...
    def has_cycle_helper(self, currentVertex, notVisited, currentlyVisiting, visited):
        notVisited.remove(currentVertex)

        currentlyVisiting.append(currentVertex)

        neighbors = self.get_neighbors(currentVertex.label)
        ...
        for vertex in neighbors:
            ...
            if (vertex in currentlyVisiting):
                print()
                return True

            if (self.has_cycle_helper(vertex, notVisited, currentlyVisiting, visited)):
                return True
    ...
    def has_cycle (self):
        ...
        notVisited = self.get_vertices()
        print(notVisited)
        currentlyVisiting = []
        visited = []

        while (len(notVisited) > 0):
            starting_vertex = notVisited[0]

            if (self.has_cycle_helper(starting_vertex, notVisited, currentlyVisiting, visited)):
                return True

        return False

    # return a list of vertices after a topoligcal sort
...
def main():
    ...
    for i in range (num_edges):
        ...
        start = edge[0]
        finish = edge[1]
        start_index = letters.get_index(start)
        finish_index = letters.get_index(finish)
        letters.add_undirected_edge (start_index, finish_index)

    # printing adjacency matrix:
    print ("Adjacency Matrix")
    print()
    for i in range(letters.num_vertices):
        for j in range(letters.num_vertices - 1):
            print(letters.adjMat[i][j], end = ' ')
        print(letters.adjMat[i][letters.num_vertices - 1])

    print()

    print(letters.has_cycle())
