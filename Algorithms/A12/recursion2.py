...
#  Description: Assignment 12, more recursion problems
...
def groupSum5(start, nums, target):
	if (start >= len(nums)): #base case 1
		return (target == 0)
	elif (start == len(nums) - 1): #base case 2
		return (target == 0) or (target - nums[start] == 0)
	elif ((nums[start] % 5) == 0): #multiple of 5
		if (nums[start + 1] == 1): #following number is 1
			return groupSum5(start + 2, nums, target - nums[start]) #skip the 1
...
def groupSumClump(start, nums, target):
	if (start >= len(nums)):
		...
		return (target == 0) or (target - nums[start] == 0)
	elif (nums[start] == nums[start + 1]): #adjacent numbers are identical
		i = 1
		for j in range(1, len(nums) - start):
...
def splitArray(nums):
	...
	b = [] #second group
	idx = 0
	return splitArrayHelper(nums, a, b, idx)



def splitArrayHelper(nums, a, b, idx):
	if idx == len(nums):
		return sum(a) == sum(b)
	else:
		c = a[:] # copy of first half
		c.append(nums[idx])
		d = b[:] #copy of second half
		d.append(nums[idx])
		return splitArrayHelper(nums, c, b, idx + 1) or splitArrayHelper(nums, a, d, idx + 1)
...
def splitOdd10(nums):
	...
	b = []
	idx = 0
	return splitOdd10Helper(nums, a, b, idx)


def splitOdd10Helper(nums, a, b, idx):
	if idx == len(nums):
		return (sum(a) % 10 == 0 and sum(b) % 2 != 0) or (sum(a) % 2 != 0 and sum(b) % 10 == 0)
	else:
		c = a[:]
		c.append(nums[idx])
		d = b[:]
		d.append(nums[idx])
		return splitOdd10Helper(nums, c, b, idx + 1) or splitOdd10Helper(nums, a, d, idx + 1)
...
def split53(nums):
	...
	b = [] #multiples of 3
	idx = 0
	return split53Helper(nums, a, b, idx)

def split53Helper(nums, a, b, idx):
	if (idx == len(nums)):
		return sum(a) == sum(b)
	elif (nums[idx] % 5 == 0):
		a.append(nums[idx])
		return split53Helper(nums, a, b, idx + 1)
	elif (nums[idx] % 3 == 0) and (nums[idx] % 5 != 0): #multiples of 5 are gone
		b.append(nums[idx])
		return split53Helper(nums, a, b, idx + 1)
	else:
		c = a[:]
		c.append(nums[idx]) # multiples of 5
		d = b[:]
		d.append(nums[idx]) # multiples of 3
		return split53Helper(nums, c, b, idx + 1) or split53Helper(nums, a, d, idx + 1)
